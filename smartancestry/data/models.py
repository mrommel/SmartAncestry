...
class Person(models.Model):
    ...
    def __unicode__(self):              
        return mark_safe('%s %s' % ((' ' + str(self.first_name) + ' ').replace(" _", " <u>").replace("_ ", "</u> "), self.last_name))

class TimelineInfo(object):
    def __init__(self, date, title):
        self.date = date
        self.title = mark_safe((' ' + title + ' ').replace(" _", " <u>").replace("_ ", "</u> "))
...
class Ancestry(models.Model):
	...
	def statistics(self):
		...
		# todo: sort & limit birth location to 10 (+rest)
		birthLocations.limit(10)
		
		specials.add(mark_safe("Youngest person:</td><td>%s" % youngestPerson), youngestAge)
		specials.add(mark_safe("Oldest person:</td><td>%s" % oldestPerson), oldestAge)
		specials.add(mark_safe("Lastest marriage:</td><td>%s" % latestMarriagePerson), latestMarriageAge)
		specials.add(mark_safe("Youngest marriage:</td><td>%s" % youngestMarriagePerson), youngestMarriageAge)
